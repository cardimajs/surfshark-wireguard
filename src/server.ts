...
server.get("/connect/:serverHostname", async (request, reply) => {
  ...
  const selectedServer = surfshark
    .getServers()
    .find((server) => server.hostname === serverHostname);
  ...
  await wireguard.setConfig(wireGuardConfig);
  try {
    await surfshark.sendPublicKey(wireguard?.myPublickey);
  } catch (err) {
    console.log("Deu ruim ao enviar public key");
  }
  await wireguard.restart();
